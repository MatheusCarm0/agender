...
interface Customization {
  ...
  links: {
    ...
    thumbnailUrl?: string;
    style?: string;
    type?: "link" | "heading" | "divider" | "text" | "spacer";
  }[];
  socials: {
    instagram?: string;
    whatsapp?: string;
...
function bgStyle(
  bg: Customization["theme"]["background"],
  overlayOpacity?: number,
): React.CSSProperties {
  ...
  try {
    ...
    for (let offset = 0; offset < 7; offset++) {
      ...
      if (nextHours.length > 0) {
        if (
          offset === 0 &&
          nextHours.some((wh) => currentTime < wh.startTime)
        ) {
          const next = nextHours.find((wh) => currentTime < wh.startTime)!;
          return {
            isOpen: false,
            label: `Fechado · Abre às ${next.startTime}`,
          };
        }
        if (offset > 0) {
          ...
          return {
            isOpen: false,
            label: `Fechado · Abre ${dayLabel} às ${nextHours[0].startTime}`,
          };
        }
      }
    }
...
const PERIOD_CONFIG = [
  { key: "manha", label: "Manhã", test: (h: number) => h < 12 },
  { key: "tarde", label: "Tarde", test: (h: number) => h >= 12 && h < 18 },
  { key: "noite", label: "Noite", test: (h: number) => h >= 18 },
] as const;

const VISIBLE_LIMIT = 8;

function groupSlotsByPeriod(slots: Slot[]) {
  const groups: Record<string, Slot[]> = { manha: [], tarde: [], noite: [] };
  for (const slot of slots) {
    const hour = new Date(slot.startAt).getHours();
    const period = PERIOD_CONFIG.find((p) => p.test(hour))!.key;
        ...
        return (
          ...
          >
            ...
            {i > 0 && (
              <div
                ...
                style={{
                  left: "calc(-50% + 18px)",
                  right: "calc(50% + 18px)",
                  backgroundColor: reached ? primary : `${primary}25`,
                }}
              />
            )}
            <div
              className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold relative z-10 transition-all"
              style={{
            ...
            <span
              ...
              style={{ color: isActive ? accent : `${primary}60` }}
            >
              {s.label}
            </span>
          </div>
...
export default function BookingClient({
  ...
  useEffect(() => {
    ...
    function onMsg(e: MessageEvent) {
      ...
      if (d && d.type === "agender:preview" && d.payload) {
        ...
        setOverrideLogo(
          typeof d.payload.logoUrl === "string" ? d.payload.logoUrl : null,
        );
      }
    }
  ...
  const dates = generateDates(weekOffset); // ex: weekOffset*7 dias à frente
  ...
  const payRemainingSec = payInfo?.expiresAt
    ? Math.max(
        0,
        Math.floor((new Date(payInfo.expiresAt).getTime() - nowTs) / 1000),
      )
    : null;
  ...
  async function startPixPayment() {
    ...
    try {
      const res = await fetch(
        `${API_URL}/public/v1/${business.slug}/appointments/${appointmentId}/pay`,
        {
          method: "POST",
      ...
      setPayError(
        err instanceof Error ? err.message : "Erro ao gerar o pagamento",
      );
    }
    setPayLoading(false);
  }
  ...
  async function checkPaymentStatus() {
    ...
    try {
      const res = await fetch(
        `${API_URL}/public/v1/${business.slug}/appointments/${appointmentId}/pay/status`,
      );
      if (!res.ok) return;
      const data = await res.json();
  ...
  const cardRad =
    cust?.theme?.buttonStyle === "square"
      ? "6px"
      : cust?.theme?.buttonStyle === "pill"
        ? "18px"
        : "14px";
  ...
  const hoursByDay = (workingHours || []).reduce<Record<number, WorkingHour[]>>(
    (acc, wh) => {
  ...
  const openStatus =
    workingHours && workingHours.length > 0
      ? getOpenStatus(workingHours, business.timezone)
      : null;
  ...
  const appliedDiscount = couponStatus?.valid
    ? (couponStatus.discountAmount ?? 0)
    : 0;
  const bookingTotal = selectedService
    ? Math.max(0, selectedService.price - appliedDiscount)
    : 0;
  ...
  const expectedCharge =
    paymentPolicy === "full"
      ...
      : paymentPolicy === "deposit"
        ? Math.round(
            bookingTotal * ((business.depositPercent ?? 0) / 100) * 100,
          ) / 100
        : 0;
  ...
  async function validateCoupon() {
    ...
    try {
      const res = await fetch(
        `${API_URL}/public/v1/${business.slug}/coupons/preview`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!res.ok) {
        setCouponStatus({
          valid: false,
          message: body.message || "Cupom inválido ou expirado",
        });
      } else {
        setCouponStatus({
          valid: true,
      ...
      setCouponStatus({
        valid: false,
        message: "Não foi possível validar o cupom. Tente de novo.",
      });
    }
  ...
  async function handleBook(e: React.FormEvent) {
    e.preventDefault();
    if (preview) {
      setError("Pré-visualização: o agendamento fica desativado aqui.");
      return;
    }
    ...
    try {
      const res = await fetch(
        `${API_URL}/public/v1/${business.slug}/appointments`,
        {
          method: "POST",
          headers: {
  ...
  function goHome() {
    ...
    setSelectedService(null);
    setSelectedSlot(null);
    setClientForm({ name: "", phone: "", email: "", marketingOptIn: false });
    setError("");
    setCouponCode("");
    setCouponStatus(null);
...
function generateDates(pageOffset: number): {
  date: string;
  label: string;
  dayName: string;
  isToday: boolean;
  ...
  for (let i = 0; i < DAYS_TO_SHOW; i++) {
    const dayIndex = pageOffset * DAYS_TO_SHOW + i; // desloca a janela inteira por página
    ...
    dates.push({
      ...
      isToday: dayIndex === 0, // não pode ser `i === 0` mais
    });
  }
  return dates;
}
      ...
      >
        ...
        {showHours && openStatus && openStatus.label && (
          <button
            ...
            aria-label={
              hoursExpanded
                ? "Ocultar horário de funcionamento"
        ...
        {hoursExpanded && workingHours && workingHours.length > 0 && (
          ...
          >
            {[1, 2, 3, 4, 5, 6, 0].map((day) => {
              ...
              const ranges = hours
                ? Array.from(
                    new Set(hours.map((h) => `${h.startTime} - ${h.endTime}`)),
                  )
                : [];
              return (
                <div key={day} className="flex justify-between py-0.5">
                  <span style={{ opacity: 0.6 }}>{DAY_NAMES[day]}</span>
                  <span style={{ opacity: ranges.length ? 0.8 : 0.4 }}>
                    {ranges.length ? ranges.join(", ") : "Fechado"}
                  </span>
                </div>
              );
            })}
          ...
          <div className="flex gap-3 mt-5 stagger-item">
            {socials!.instagram && (
              <a
                ...
                style={{ backgroundColor: `${text}10`, opacity: 0.7 }}
              >
                <svg
                  width="18"
                  height="18"
      ...
      >
        ...
        {step === "home" && (
          <div className="w-full max-w-md space-y-5 step-animate">
            {cust?.welcomeMsg && (
              <p
                className="text-center text-[13px] font-medium tracking-wide mb-1 stagger-item"
            ...
            {business.acceptingBookings === false ? (
              <div
                className="text-center py-4 px-4 rounded-lg stagger-item"
                style={{
              ...
              >
                ...
                <p className="text-xs mt-1" style={{ opacity: 0.5 }}>
                  Este negócio não está aceitando novos agendamentos no momento.
                </p>
              </div>
            ) : (
              <button
                onClick={() => navigate("select")}
                className="w-full py-4 text-sm font-semibold tracking-wide cta-btn stagger-item"
                style={{
                  backgroundColor: primary,
                  color: onPrimary,
                  borderRadius: radius,
                  ["--pub-primary-glow" as string]: `${primary}50`,
                }}
              >
                Agendar horário
              </button>
            ...
            {gallery && gallery.length > 0 && (
              <div className="pt-5 -mx-6 stagger-item relative group">
                <div
                  ref={galleryRef}
                  className="flex gap-2.5 overflow-x-auto px-6 pb-1 snap-x scrollbar-none"
            ...
            {hasAddress && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-start gap-2 pt-4 text-xs no-underline transition-opacity hover:opacity-80 stagger-item"
                style={{ color: text, opacity: 0.55 }}
              >
                <svg
                  width="14"
        ...
        {step === "select" && (
          <div className="w-full max-w-md space-y-5 step-animate">
            ...
            {business.professionals.length === 0 ? (
              <p className="text-center py-12 text-sm" style={{ opacity: 0.5 }}>
            ...
            >
              <div className="mb-4">
                ...
                <p className="text-xs" style={{ opacity: 0.5 }}>
                  com {selectedProf?.name} · {selectedService?.durationMin} min
                  ·{" "}
                  {selectedService ? formatCurrency(selectedService.price) : ""}
                </p>
              </div>

              {/* Date strip paginada por semana */}
              <div className="flex items-center gap-1 mb-1">
                <button
                  ...
                  className="shrink-0 h-7 w-6 flex items-center justify-center rounded-full transition-opacity disabled:opacity-20"
                  style={{ opacity: 0.5 }}
                >
                  <svg
                    width="14"
                ...
                <div
                  className="flex gap-1.5 overflow-x-auto pb-3 px-1 scrollbar-none flex-1"
                  style={{ scrollbarWidth: "none" }}
                >
                  {dates.map((d) => (
                    <button
                      ...
                      style={{
                        backgroundColor:
                          selectedDate === d.date ? primary : `${text}06`,
                    ...
                    >
                      <span
                        ...
                        style={{ opacity: selectedDate === d.date ? 1 : 0.5 }}
                      >
                        {d.dayName}
                      </span>
                ...
                <button
                  ...
                  className="shrink-0 h-7 w-7 flex items-center justify-center rounded-full transition-opacity"
                  style={{ opacity: 0.5 }}
                >
                  <svg
                    width="14"
              ...
              {loadingSlots ? (
                ...
                PERIOD_CONFIG.map(({ key, label }) => {
                  const periodSlots = grouped[key];
                  if (periodSlots.length === 0) return null;
                  const isExpanded = expandedPeriods[key];
                  const visible = isExpanded
                    ? periodSlots
                    : periodSlots.slice(0, VISIBLE_LIMIT);
                  const hidden = periodSlots.length - visible.length;
            ...
            >
              ...
              <div className="space-y-1 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {selectedSlot &&
                      new Date(selectedSlot.startAt).toLocaleString("pt-BR", {
                        weekday: "short",
                        day: "2-digit",
                        month: "short",
                        hour: "2-digit",
                ...
                {appliedDiscount > 0 && (
                  <div className="flex justify-between">
                    <span style={{ opacity: 0.7 }}>
                      Desconto (
                      {couponStatus?.valid ? couponCode.toUpperCase() : ""})
                    </span>
              ...
              {paymentPolicy !== "none" && expectedCharge > 0 && (
                <p
                  className="text-xs mt-3 px-3 py-2 rounded-lg flex items-start gap-1.5"
                  style={{ backgroundColor: `${primary}12`, color: accent }}
                >
                  <svg
                    width="13"
                    height="13"
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 h-11 px-3 text-sm border focus:outline-none uppercase tracking-wider"
                      style={{
                        backgroundColor: surface,
                        borderColor: `${text}20`,
                        color: text,
                    ...
                    <button
                      ...
                      className="h-11 px-4 text-sm font-medium border disabled:opacity-40"
                      style={{
                        borderColor: `${text}20`,
                        color: text,
                        borderRadius: radius,
                  ...
                  {couponStatus && (
                    <p
                      ...
                      style={{
                        backgroundColor: couponStatus.valid
                          ? "#f0fdf4"
                          : "#fef2f2",
                        color: couponStatus.valid ? "#16a34a" : "#dc2626",
                      }}
                    >
            ...
            >
              <div className="text-center mb-5">
                ...
                {paymentPolicy === "deposit" && bookingTotal > payDue && (
                  <p className="text-xs mt-1" style={{ opacity: 0.55 }}>
                    O restante ({formatCurrency(bookingTotal - payDue)}) você
                    paga no local.
                  </p>
                )}
              </div>
                    ...
                    >
                      {(
                        ...
                        <button
                          ...
                          style={{
                            backgroundColor:
                              payMethod === m ? surface : "transparent",
                ...
                <div className="space-y-4">
                  ...
                  {payRemainingSec !== null && payRemainingSec > 0 && (
                    ...
                    >
                      ...
                      {formatCountdown(payRemainingSec)}
                    </p>
                  )}
                  <button
                    type="button"
                ...
                (payInfo.status === "expired" ||
                  ...
                  <div className="text-center space-y-3">
                    <p
                      ...
                      style={{ backgroundColor: "#fef2f2", color: "#b91c1c" }}
                    >
                      O pagamento não foi concluído a tempo e a reserva foi
                      liberada.
                    </p>
                    <button
                      onClick={goHome}
              ...
              {(!payInfo || payInfo.status === "pending") && (
                <div
                  ...
                  style={{ borderTop: `1px solid ${text}10` }}
                >
                  <button
                    onClick={goHome}
                    className="text-xs transition-opacity hover:opacity-80"
                    style={{ opacity: 0.5, color: text }}
                  >
        ...
        {step === "done" && (
          <div className="w-full max-w-md space-y-5 step-animate">
            ...
            >
              <div
                className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-5 check-animate"
              ...
              {hasAddress && (
                <div
                  ...
                  style={{ backgroundColor: `${text}05` }}
                >
                  <p className="font-medium mb-0.5" style={{ opacity: 0.6 }}>
                    Endereço
                  </p>
                  <a
                    href={mapsUrl}
              ...
              <div className="flex flex-col gap-2">
                ...
                {socials?.whatsapp && (
                  <a
                    href={`https://wa.me/?text=${encodeURIComponent(`Agendei ${selectedService?.name} na ${business.name}! ${selectedSlot ? new Date(selectedSlot.startAt).toLocaleString("pt-BR", { weekday: "long", day: "2-digit", month: "long", hour: "2-digit", minute: "2-digit" }) : ""}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full py-3 text-sm font-medium border flex items-center justify-center gap-2 transition-all hover:scale-[1.02]"
                    style={{
                ...
                <button
                  onClick={goHome}
                  className="w-full py-3 text-sm font-medium transition-all hover:scale-[1.02]"
                  style={{ color: accent }}
                >
                ...
                <a
                  href={`/${business.slug}/conta`}
                  className="w-full py-1 text-xs font-medium text-center transition-all hover:opacity-80"
                  style={{ opacity: 0.55, color: text }}
                >
      ...
      {lightboxIndex !== null && gallery && (
        ...
        >
          <button
            type="button"
            onClick={() => setLightboxIndex(null)}
            className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 text-white flex items-center justify-center text-xl hover:bg-white/20"
            aria-label="Fechar"
          >
            ×
          </button>
