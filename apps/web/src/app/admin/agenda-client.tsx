...
export default function AgendaClient({ initialAppointments }: { initialAppointments: Appointment[] | null }) {
  ...
  return (
    <div>
                    ...
                    <td className="px-4 py-3 text-right">
                      {appt.status === 'scheduled' && (
                        <div className="flex gap-1 justify-end">
                          <button onClick={() => updateStatus(appt.id, 'confirmed')} className="text-xs px-2 py-1 text-primary-default hover:bg-surface-subtle rounded-[var(--radius-sm)]">Confirmar</button>
                          <button onClick={() => setCancelTarget(appt.id)} className="text-xs px-2 py-1 text-danger-fg hover:bg-surface-subtle rounded-[var(--radius-sm)]">Cancelar</button>
                        </div>
                      )}
                      {appt.status === 'confirmed' && (
                        <div className="flex gap-1 justify-end">
                          <button onClick={() => setCompleteTarget(appt.id)} className="text-xs px-2 py-1 text-success-fg hover:bg-surface-subtle rounded-[var(--radius-sm)]">Concluir</button>
                          <button onClick={() => setNoShowTarget(appt.id)} className="text-xs px-2 py-1 text-danger-fg hover:bg-surface-subtle rounded-[var(--radius-sm)]">Não compareceu</button>
                        </div>
                      )}
                    </td>
                  </tr>
      ...
      <ConfirmModal open={!!cancelTarget} title="Cancelar agendamento" description="Tem certeza que deseja cancelar este agendamento? O cliente será notificado e o horário ficará disponível." confirmLabel="Cancelar agendamento" variant="danger"
